...
ok(context.spcDevSenderSurfaceAllowed() === false, 'desktop/unknown platform is not an allowed sender surface');
ok(context.spcDevSenderCardHtml(false) === '', 'desktop/unknown platform still renders no card even with gate');
context.hostPlatform = 'android';
ok(context.spcDevSenderSurfaceAllowed() === true, 'android platform is an allowed sender surface');
ok(context.spcDevSenderCardHtml(false).includes('data-qa="spc-dev-sender"'), 'enabled card renders');

section('validation');
context.hostPlatform = 'linux';
let bad = await context.spcDevSend(goodCfg(), file);
ok(bad.ok === false && bad.state === 'mobile_only', 'desktop sender is blocked at submit layer');
context.hostPlatform = 'android';
bad = await context.spcDevSend({ ...goodCfg(), apiBase: 'https://api.skipi.app' }, file);
