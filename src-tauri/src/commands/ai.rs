...
pub fn ai_recognize(
    ...
    let tpl_key: String = doc.template_id.clone().unwrap_or_else(|| doc.category.clone());
    ...
    let prompt = format!("{}{}{}", template_text, base_prompt, corrections_text);
    ...
    if use_local {
        ...
        let resp = client
            ...
            .json(&body)
            .send()
            .map_err(|e| format!("Ollama request failed ({}): {}. Is Ollama running?", ollama_url, e))?;
        ...
        content_text = resp_json["content"][0]["text"].as_str().unwrap_or("{}").to_string();
    }
    ...
    let json_str = if let Some(start) = content_text.find('{') {
        if let Some(end) = content_text.rfind('}') {
            ...
            let cleaned: String = raw.lines().map(|line| {
    ...
    let result: AiRecognizeResult = match serde_json::from_str::<serde_json::Value>(&json_str) {
        Ok(mut v) => {
        ...
        Err(e) => return Err(format!("Cannot parse AI response: {} — raw: {}", e, json_str)),
    };
...
pub fn get_api_key(state: State<AppState>) -> Result<String, String> {
    let lock = state.conn.lock().unwrap_or_else(|e| e.into_inner());
    let conn = lock.as_ref().ok_or("No vault open")?;
    let info_key: String = conn.query_row(
...
#[tauri::command]
pub fn update_field_statuses(state: State<AppState>, id: String, statuses: String) -> Result<(), String> {
    let lock = state.conn.lock().unwrap_or_else(|e| e.into_inner());
    let conn = lock.as_ref().ok_or("No vault open")?;
...
#[tauri::command]
pub fn save_ai_correction(state: State<AppState>, doc_type: String, field_name: String, ai_value: String, correct_value: String) -> Result<(), String> {
    let lock = state.conn.lock().unwrap_or_else(|e| e.into_inner());
    let conn = lock.as_ref().ok_or("No vault open")?;
    db::save_correction(conn, &doc_type, &field_name, &ai_value, &correct_value).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn get_ai_corrections(state: State<AppState>, doc_type: String) -> Result<Vec<(String, String, String)>, String> {
    let lock = state.conn.lock().unwrap_or_else(|e| e.into_inner());
    let conn = lock.as_ref().ok_or("No vault open")?;
