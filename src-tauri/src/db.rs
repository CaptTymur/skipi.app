    ...
    mod cv_order_282_tests {
        ...
        fn differential_database_outputs() {
            ...
            for case in cases {
                ...
                let baseline_ids = conn
                    .prepare(
                        "SELECT id FROM work_history ORDER BY COALESCE(sign_on,created_at) DESC",
                    )
                    .unwrap()
        ...
        fn real_assistant_reader_runtime() {
            ...
            let server = std::thread::spawn(move || {
                ...
                let reply =
                    r#"{"reply":"synthetic stub","model":"local-fixture","remaining_today":1}"#;
                ...
                .all(|p| std::path::Path::new(p).starts_with(&root)));
            {
                ...
                for id in ["old", "undated", "new"] {
                    let dir =
                        crate::commands::work_history::work_entry_storage_dir(&root, conn, id)
                            .unwrap();
            ...
            .unwrap();
            let downloads =
                std::path::PathBuf::from(std::env::var("SKIPI_282_DOWNLOAD_DIR").unwrap());
            ...
            assert!(
                profile.find("SYNTHETIC-new").unwrap() < profile.find("SYNTHETIC-old").unwrap()
            );
            assert!(
