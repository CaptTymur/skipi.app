...
/// Build a simple one-page A4 demo PDF.
///
/// Keep this on the same `printpdf` path as CV/package generation instead of
/// hand-assembling PDF objects. Android viewers are stricter than desktop PDF
/// readers and may show a blank page for very small synthetic PDFs.
pub fn build_demo_pdf(title: &str) -> Vec<u8> {
    ...
    let (doc, page1, layer1) =
        PdfDocument::new("Skipi Demo Document", Mm(210.0), Mm(297.0), "Page 1");
    let layer = doc.get_page(page1).get_layer(layer1);
    let regular = match doc.add_builtin_font(BuiltinFont::Helvetica) {
        Ok(font) => font,
        Err(_) => return fallback_demo_pdf(&clean_title),
    };
    ...
    layer.set_fill_color(Color::Rgb(Rgb::new(0.10, 0.10, 0.12, None)));
    layer.use_text("Skipi DEMO", 24.0, Mm(24.0), Mm(260.0), &bold);
    layer.use_text(&clean_title, 16.0, Mm(24.0), Mm(244.0), &bold);

    layer.set_fill_color(Color::Rgb(Rgb::new(0.40, 0.40, 0.46, None)));
    layer.use_text(
        "Synthetic training document for mobile vault testing.",
        11.0,
        Mm(24.0),
        Mm(230.0),
        &regular,
    ...
    layer.use_text(
        "Replace it with a real scan or photo before sending documents.",
        11.0,
        Mm(24.0),
        Mm(222.0),
        &regular,
    ...
    layer.set_fill_color(Color::Rgb(Rgb::new(0.25, 0.22, 0.75, None)));
    layer.use_text(
        "This PDF is generated by Skipi and should open in Android PDF viewers.",
        10.0,
        Mm(24.0),
        Mm(38.0),
        &regular,
    ...
    let mut bytes = Vec::new();
    {
        ...
        if doc.save(&mut writer).is_err() {
            return fallback_demo_pdf(&clean_title);
        }
    }
...
fn fallback_demo_pdf(title: &str) -> Vec<u8> {
